if game.PlaceId == 7465136166 then -- GPO (Second Sea)

  loadstring(game:HttpGet("https://raw.githubusercontent.com/OzNL-Hub/OzNL-Hub/main/GPO_SecondWorld.lua"))()

elseif game.PlaceId == 3978370137 then -- GPO (First Sea)

  loadstring(game:HttpGet("https://raw.githubusercontent.com/OzNL-Hub/OzNL-Hub/main/GPO_FirstSea.lua"))()

elseif game.PlaceId == 11424731604 then -- GPO (Impel Down)

  loadstring(game:HttpGet("https://raw.githubusercontent.com/OzNL-Hub/OzNL-Hub/main/Impel-Down.lua"))()

elseif game.PlaceId == 4855457388 or game.PlaceId == 5094651510 then -- Demonfall

  loadstring(game:HttpGet("https://raw.githubusercontent.com/OzNL-Hub/OzNL-Hub/main/Demonfall.lua"))()

elseif game.PlaceId == 9224601490 then -- Fruit Battle Grounds (Main World)

  loadstring(game:HttpGet("https://raw.githubusercontent.com/OzNL-Hub/OzNL-Hub/main/FruitBattleGrounds.lua"))()

elseif game.PlaceId == 17387482786 or game.PlaceId == 13883059853 then -- Project Slayers (World2)

  loadstring(game:HttpGet("https://raw.githubusercontent.com/OzNL-Hub/OzNL-Hub/main/ProjectSlayersWorld2.lua"))()

elseif game.PlaceId == 11468034852 then -- Project Slayers (Mugen Train)

  loadstring(game:HttpGet("https://raw.githubusercontent.com/OzNL-Hub/OzNL-Hub/main/ProjectSlayersMugenTrain.lua"))()

elseif game.PlaceId == 185655149 then -- Blox Burg

  loadstring(game:HttpGet("https://raw.githubusercontent.com/OzNL-Hub/OzNL-Hub/main/Bloxburg.lua"))()

end
